"""Configuration for Smpl velocity tracking task on rough terrain."""
...
from genesis_assets.robots import SMPL_HUMANOID_CFG
import genesis_tasks.locomotion.velocity.mdp as mdp


@configclass
class SMPLRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    """Configuration for Smpl velocity tracking on rough terrain."""
    ...
    def __post_init__(self):
        ...
        if isinstance(self.scene, type(MISSING)) or self.scene is MISSING or self.scene is None:
            self.scene = SceneCfg(
                ...
                backend="cuda",
                viewer=False,
                robots={"robot": SMPL_HUMANOID_CFG},
...
@configclass
class SMPLRoughEnvCfg_PLAY(SMPLRoughEnvCfg):
    """Configuration for Smpl velocity tracking on rough terrain (play mode)."""
