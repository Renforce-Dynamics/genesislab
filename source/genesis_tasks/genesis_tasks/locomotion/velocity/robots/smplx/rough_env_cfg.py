"""Configuration for Smplx velocity tracking task on rough terrain."""
...
from genesis_assets.robots import SMPLX_HUMANOID_CFG
import genesis_tasks.locomotion.velocity.mdp as mdp


@configclass
class SMPLXRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    """Configuration for Smplx velocity tracking on rough terrain."""
    ...
    def __post_init__(self):
        ...
        if isinstance(self.scene, type(MISSING)) or self.scene is MISSING or self.scene is None:
            self.scene = SceneCfg(
                ...
                backend="cuda",
                viewer=False,
                robots={"robot": SMPLX_HUMANOID_CFG},
...
@configclass
class SMPLXRoughEnvCfg_PLAY(SMPLXRoughEnvCfg):
    """Configuration for Smplx velocity tracking on rough terrain (play mode)."""
