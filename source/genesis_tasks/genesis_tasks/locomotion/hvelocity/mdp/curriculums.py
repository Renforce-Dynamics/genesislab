...
from .commands import UniformLevelVelocityCommandCfg, UniformVelocityCommandCfg
...
def lin_vel_cmd_levels(
    ...
    num_steps: int = 8_000_000,
) -> torch.Tensor:
    """Interpolate command sampling ranges from their initial values toward ``limit_ranges``.

    The initial ranges are captured once on the command term (first call) so
    curriculum always expands from the task's starting distribution.
    """
    del env_ids
    ...
    if command_term is None:
        raise ValueError(
            f"Command term '{command_name}' not found. Available: {list(env.command_manager._terms.keys())}"
        )

    cfg = command_term.cfg
    if not hasattr(cfg, "limit_ranges"):
        return torch.tensor(0.0, device=env.device, dtype=torch.float32)

    cfg = cast(UniformLevelVelocityCommandCfg, cfg)
    if not hasattr(command_term, "_hvelocity_cmd_init_ranges"):
        command_term._hvelocity_cmd_init_ranges = copy.deepcopy(cfg.ranges)

    t = min(1.0, float(env.common_step_counter) / float(max(1, num_steps)))
    init = command_term._hvelocity_cmd_init_ranges
    lim = cfg.limit_ranges

    cfg.ranges.lin_vel_x = _lerp_range(init.lin_vel_x, lim.lin_vel_x, t)
    cfg.ranges.lin_vel_y = _lerp_range(init.lin_vel_y, lim.lin_vel_y, t)
    cfg.ranges.ang_vel_z = _lerp_range(init.ang_vel_z, lim.ang_vel_z, t)
    if cfg.heading_command and init.heading is not None and lim.heading is not None:
        cfg.ranges.heading = _lerp_range(init.heading, lim.heading, t)

    return torch.tensor(t, device=env.device, dtype=torch.float32)
